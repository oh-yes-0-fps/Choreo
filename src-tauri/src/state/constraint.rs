...
use super::{utils::sqlx_stringify, waypoint::WaypointScope};
...
impl Constraint {
    ...
    pub fn definition(data: ConstraintData) -> ConstraintDefinition {
        match data {
            ConstraintData::WptVelocityDirection { direction: _ } => {
                Constraints.wpt_velocity_direction
            }
...
pub enum ConstraintData {
    WptVelocityDirection { direction: f64 },
    WptZeroVelocity,
    StopPoint,
    MaxVelocity { velocity: f64 },
    ZeroAngularVelocity,
    StraightLine,
    PointAt { x: f64, y: f64, tolerance: f64 },
}
...
pub async fn get_path_constraints(
    pool: &Pool<Sqlite>,
    path_id: &i64,
...
pub async fn add_constraint(
    ...
    row
        .insert_raw(pool, "constraints")
        .await
        .map(|_| row.constraint_id)
}
